...
' =============================================
' Synchronisation BDD-DOC <-> Dossier
...
' - zDocConstance importé en premier
' - .bas / .cls / .frm importés en natif avec remplacement transactionnel
' - nettoyage des composants temporaires tmpOld_... avant import
' - Base.txt / ThisWorkbook.txt remplacés en place
' - zDocGitSync et UF_GitSync ne sont jamais réimportés automatiquement
' - Journal disque : %TEMP%\BDD_DOC_IMPORT_DEBUG.txt
' =============================================
...
Private Const NOM_MODULE_UTILITAIRE As String = "zDocGitSync"
Private Const NOM_USERFORM_GITSYNC As String = "UF_GitSync"

Private Const MODULES_PRIORITAIRES As String = "zDocConstance"
...
Private Function FichierImportDangereux(ByVal cheminFichier As String) As Boolean
    ...
    If InStr(1, contenuUpper, "SYNCHRONISATION BDD-DOC <-> DOSSIER", vbTextCompare) > 0 Then
...
Private Sub JournaliserIO(ByVal contexte As String, _
    ...
    Debug.Print Format$(Now, "yyyy-mm-dd hh:nn:ss") & " [zDocGitSync] " & contexte & _
...
Private Sub JournaliserImportCrash(ByVal etape As String, Optional ByVal detail As String = "")
    ...
    cheminLog = Environ$("TEMP") & "\BDD_DOC_IMPORT_DEBUG.txt"
    numFichier = FreeFile
...
Public Sub VerifierPreRequisGitSync()
    ...
    message = "=== Pré-check GitSync BDD-DOC ===" & vbCrLf & vbCrLf
...
Private Function TesterEcritureDossier(ByVal dossier As String) As Boolean
    ...
    cheminTest = dossier & "\__zDocGitSync_write_test__.tmp"
